...
[@dvm/copilot-room] session 2f1c…  ready
```
...
Every entry point reads it - `npm run dev`, `npx @dvm/copilot-room`, and the
...
environment beats the file, so `ENTRA_CLIENT_SECRET=... npx @dvm/copilot-room`
